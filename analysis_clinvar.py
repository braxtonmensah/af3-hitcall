"""PREREG_CLINVAR.md: pathogenic-vs-benign enrichment at predicted interfaces, by group."""
import json
import os
import re
...
recs = [json.loads(l) for l in open(os.path.join(B, "interfaces.jsonl"))]
...
t2 = res["precedented/control"]["ci95"][0] > 1
t1 = res["novel/control"]["ci95"][0] > 1
...
json.dump(res, open("results_clinvar.json", "w"), indent=1, default=float)
